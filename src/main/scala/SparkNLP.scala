object SparkNLP {
  def main(args: Array[String]) {
    ...
    //break up the dataset, 32 workers, 3 cores per to leverage Spark
    val df_partition = df.repartition(4 * 32)
    ...
    //annotates the dataset, allowing other NLP annotators and transformer to manipulate it
    ...
    //################ SECOND PIPELINE
    //Load it from the HDFS as the model download fails
    val sentiment_model = PipelineModel.load("hdfs://atlanta:30215/sentiment/analyze_sentiment_en_2.4.0_2.4_1580483464667")
    ...
    final_result.write.csv("hdfs://atlanta:30215/project/results/sentiment_analysis_Question_out_repartition")
